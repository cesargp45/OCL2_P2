...
export class ArrayExpr extends Expression {
    ...
    compile(enviorement: Enviorement): Retorno {
        ...
        this.values.forEach((value,index)=>{
            ...
            //hacer booleanos
            if(result.type.type == Types.BOOLEAN){
                if(result.getValue()){
                    const temp = generator.newTemporal();
                    result.getValue() ? generator.addGoto(value.trueLabel) : generator.addGoto(value.falseLabel);
                    ...
                    generator.addSetHeap(tempAux,temp);
                }else{
                    const temp = generator.newTemporal();
                    result.getValue() ? generator.addGoto(value.trueLabel) : generator.addGoto(value.falseLabel);
                    const templabel = generator.newLabel();
        ...
        return new Retorno(temp,true,new Type(type,'',null,dim));
