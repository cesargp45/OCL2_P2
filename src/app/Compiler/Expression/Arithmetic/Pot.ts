...
export class Pot extends Expression{
    ...
    public compile(enviorement: Enviorement): Retorno {
            ...
            case Types.INTEGER:
                switch (right.type.type) {
                    case Types.INTEGER:
                        ...
                        const bandera = this.Native_exist("native_pot");
                        if(bandera == false){
                           this.getNative("native_pot");
                        }
        ...
        let errorN = new Error_(this.line,this.column,"Semantico",`No se puede hacer la potencia ${left.type.type} ** ${right.type.type}`);
        errores.push(errorN); 
